img_color_32x32_config = {
    "gen_mult_chs": {"pre": (1024, 512), "post": (512, ), "colors": 3},
    "disc_mult_chs": {"colors": 3, "pre": (512, ), "post": (512, 1024)},
    "enc_mult_chs": {"colors": 3, "blocks": (64, 64, 128, 128, 256, 256, 512, 512)},
    "enc_hidden": 256,
    "enc_in_mlp_dim": 512,
    "ks": 3,
    "image_size": 32,
    "latent_disc_blocks": 5,
    "latent_disc_mlp_dim": 128,
    "comb_disc_blocks": 5,
    "comb_disc_mlp_dim": 128,
    "embedding_dim": 64,
    "latent_dim": 100,
    "enc_out_dim": 100,
    "bs": 128,
}

img_color_64x64_config = {
    "gen_mult_chs": {"pre": (1024, 512, 256), "post": (128, ), "colors": 3},
    "disc_mult_chs": {"colors": 3, "pre": (128, ), "post": (256, 512, 1024)},
    "enc_mult_chs": {"colors": 3, "blocks": (64, 64, 128, 128, 256, 256)},
    "enc_hidden": 256,
    "enc_in_mlp_dim": 256,
    "ks": 3,
    "image_size": 64,
    "latent_disc_blocks": 5,
    "latent_disc_mlp_dim": 128,
    "comb_disc_blocks": 5,
    "comb_disc_mlp_dim": 128,
    "embedding_dim": 64,
    "latent_dim": 100,
    "enc_out_dim": 100,
    "bs": 64,
}

img_color_128x128_config = {
    "gen_mult_chs": {"pre": (1024, 512, 256, 128), "post": (256, ), "colors": 3},
    "disc_mult_chs": {"colors": 3, "pre": (256, ), "post": (128, 256, 512, 1024)},
    "enc_mult_chs": {"colors": 3, "blocks": (128, 128, 256, 256, 512, 512, 1024, 1024)},
    "enc_hidden": 512,
    "enc_in_mlp_dim": 512,
    "ks": 3,
    "image_size": 128,
    "latent_disc_blocks": 6,
    "latent_disc_mlp_dim": 256,
    "comb_disc_blocks": 6,
    "comb_disc_mlp_dim": 256,
    "embedding_dim": 128,
    "latent_dim": 120,
    "enc_out_dim": 120,
    "bs": 16,  # Adjusted to manage increased computational requirements
}
